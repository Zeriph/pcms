<?php
    ...
    include_once 'scripts/project.php';
    $proj = null; $doadd = false; $domod = false;
    
    function add_proj($u) {
        global $proj, $doadd, $domod;
        $proj = Project::GetNewProject();
        //-1, $_POST['ulog'], $_POST['uname'], $_POST['utype'], $_POST['umail'], $_POST['uphone'], $_POST['utheme']);
        if (Project::Exists($proj->getName())) {
            $doadd = true;
            return ('Project name \''.htmlentities($proj->getName()).'\' already exists.');
        } else {
            $domod = true;
            $err = Project::AddProject($proj);
            if ($err == '') { $u->log('added project '.$proj->getFullName().' (id:'.$proj->getID().')'); }
            else { return (htmlentities($err)); }
        }
        return '';
    }
    
    function del_proj($u, $uid) {
        global $proj, $doadd, $domod;
        $oldu = Project::GetProject($proj->getID());
        $dorem = true;
        if ($oldu->isAdmin()) {
            $dorem = false;
    ...
    function mod_proj($u, $uid, $flog) {
        global $user, $doadd, $domod;
        $oldu = User::GetUser($uid);
        if ($oldu == null) {
            return ('User id not found.');
        } else {
            $user = new User($_POST['val_mod_proj'], $_POST['ulog'], $_POST['uname'], $_POST['utype'], $_POST['umail'], $_POST['uphone'], $_POST['utheme'], $flog, $oldu->getLastLogin());
            $login = $user->getLogin();
    ...
    function print_projects($u, $proj, $doadd, $sort_type, $rev) {
        if ($u != null) {
            $stypes = array(ProjectSortType::$NAME, ProjectSortType::$CREATED, ProjectSortType::$STARTED);
            $sadmt = array(ProjectSortType::$ID);
            $sfmt = ''; $safmt = '';
            $cur = ($doadd ? '_current' : '');
            if ($u->isHR()) { // add user item
                echo '
                <div class="citem'.$cur.' b_bottom">
                    <form id="uid_n1" name="uid_n1" method="POST" action="projects.php">
                        <input type="hidden" id="val_proj" name="val_proj" value="-1" />
                        <div class="uinfo" style="cursor:pointer;" onclick="document.getElementById(\'uid_n1\').submit()">
                            <img src="img/project_add.gif" />
                            <div style="display:inline-block;font-size:18px;font-weight:bold;"><br>Add project</div>
                        </div>
                    </form>
                </div>';
                foreach ($sadmt as $st) { $safmt .= '<option value="'.$st.'"'.(($sort_type == $st) ? ' selected' : '').'>'.ProjectSortType::ToString($st).'</option>'; }
            }
            foreach ($stypes as $st) { $sfmt .= '<option value="'.$st.'"'.(($sort_type == $st) ? ' selected' : '').'>'.ProjectSortType::ToString($st).'</option>'; }
            echo '<div class="uinfo_search b_bottom">
                <form id="proj_sort" name="proj_sort" method="POST" action="projects.php">
                    Sort by:&nbsp;&nbsp;<select class="dropdown" name="sort_proj" id="sort_proj">'.$safmt.$sfmt.'</select><br>
            ...
            $fmt = '<div class="citem%s b_bottom">
                <form id="uid_%s" name="uid_%s" method="POST" action="projects.php">
                    <input type="hidden" id="val_proj" name="val_proj" value="%s" />
            ...
            if ($rev) { $projs = array_reverse($projs); }
            foreach ($projs as $usr) {
                $uid = $usr->getID();
                if ($uid > 0) {
    ...
    $sort_type = ProjectSortType::$NAME; $rev = false; $err = '';
    if (Session::Validate()) {
        ...
        if (Session::ValidateUser($u, $cpage)) {
            ...
            if ($u->isHR()) { // HR/managers can edit info and admins can edit pass/remove
                ...
                else if (isset($_POST['do_add_proj'])) {
                    $err = add_proj($u);
                }
                else if (isset($_POST['val_proj'])) {
                    if (is_numeric($_POST['val_proj'])) {
                        $err = view_proj($_POST['val_proj']);
                    } else {
                        $err = 'Invalid user specified.';
                    }
                }
                else if (isset($_POST['sort_proj'])) {
                    if (is_numeric($_POST['sort_proj'])) {
                        $sort_type = ProjectSortType::FromValue($_POST['sort_proj']);
                        $rev = isset($_POST['rev']);
                    } else {
                        $err = 'Invalid user sort specified.';
                    }
                }
                else if (isset($_POST['do_mod_proj'])) {
                    if (isset($_POST['val_mod_proj']) && is_numeric($_POST['val_mod_proj'])) {
                        $err = mod_proj($u, $_POST['val_mod_proj'], (isset($_POST['uflog']) ? $_POST['uflog'] : 0));
                    } else {
                        $err = 'Invalid user specified.';
                    }
                }
                else if (isset($_POST['do_del_proj'])) {
                    if (isset($_POST['val_del_proj']) && is_numeric($_POST['val_del_proj'])) {
                        $err = del_proj($u, $_POST['val_del_proj']);
                    } else {
                        $err = 'Invalid user specified.';
                    }
                }
...
<html><head><title>PCMS - Project Management</title><link rel="stylesheet" type="text/css" href="css/main.css" />
<?php Session::UserCssRedirOrDie($u, $redir); ?><script type="text/javascript" src="js/common.js"></script>
</head><body><div class="main"><div class="header"><div class="ltext">Project Management</div><div class="rtext">
...
<?php print_projects($u, $proj, $doadd, $sort_type, $rev); ?>
</div>
<div style="margin: auto 0; height: 100%; vertical-align:top; text-align: left; overflow: auto;">
<?php
    if ($err != '') { echo Common::JSAlert($err); }
    if ($doadd || $domod) {
        echo '<div class="profile"><center><form method="POST" action="projects.php" onsubmit="return validateForm();">';
        $usrname = substr($user->getFirstName(), 0, 1).'. '.$user->getLastName();
        if ($doadd) {
            echo'<input type="hidden" name="do_add_proj" id="do_add_proj" />';
            $usrname = 'New User';
        } else { // domod
            echo '<input type="hidden" name="do_mod_proj" id="do_mod_proj" /><input type="hidden" name="val_mod_proj" id="val_mod_proj" value="'.$user->getID().'" />';
        }
        ...
        if ($domod && $u->isAdmin()) {
            echo '
                ...
                <input type="hidden" name="val_del_proj" id="val_del_proj" value="'.$proj->getID().'" />
                <input type="hidden" name="do_del_proj" id="do_del_proj" />
